...
func Test(t *testing.T) {
	...
	terraformOptions := &terraform.Options{
		...
		TerraformDir: "../../_example/single-account",
	}
...
// This will run 'terraform init' and 'terraform application' and will fail the test if any errors occur
	...
	Id := terraform.Output(t, terraformOptions, "transit_gateway_id")
	Tags := terraform.OutputMap(t, terraformOptions, "tags")

	// Check that we get back the outputs that we expect
	assert.Equal(t, "test-clouddrove-transit-gateway", Tags["Name"])
	assert.Contains(t, Id, "tgw")
